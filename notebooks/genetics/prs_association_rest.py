...
phenotypes_rest = phenotypes_rest.merge(phenotypes['sample_id'], how='outer', indicator=True, on='sample_id')
phenotypes_rest = phenotypes_rest[phenotypes_rest['_merge']=='left_only']
phenotypes_rest['resting_hr'] = phenotypes_rest['ventricular_rate']
splits = [f'50_hrr_downsample_augment_split_{i}_prediction' for i in range(5)]
phenotypes_rest['50_hrr_downsample_augment_prediction'] = phenotypes_rest[splits].median(axis=1)

columns_to_keep = ['sample_id', '50_hrr_downsample_augment_prediction', 'resting_hr', 
                   'c_antihypertensive', 'c_lipidlowering', 'age', 'male', 'nonwhite',
                   'bmi', 'cholesterol', 'HDL', 'diastolic_bp', 'systolic_bp',
                   'current_smoker', 'creatinine', 'instance0_date', 'gfr']

phenotypes_rest[columns_to_keep].to_csv('rest_only_for_genetics.csv')
pd.concat([phenotypes[columns_to_keep], phenotypes_rest[columns_to_keep]]).to_csv('rest_exercise_for_genetics.csv')
...
for i, pheno in enumerate(['50_hrr_downsample_augment_prediction', 'ventricular_rate']):
...
plt.tight_layout()
f.savefig('hrr_prs_beta_rest.png', dpi=500)
...
ax.set_xlabel('$\\beta$ (bpm) per 1-SD increase of HRR PRS')
plt.tight_layout()
f.savefig('hrr_only_prs_beta_rest.png', dpi=500)
...
ax.set_xlabel('$\\beta$ (bpm) per 1-SD increase of HRR PRS')
plt.tight_layout()
f.savefig('hrr_shared_prs_beta_rest.png', dpi=500)
...
ax.set_xlabel('$\\beta$ (bpm) per 1-SD increase of HRR PRS')
plt.tight_layout()
f.savefig('hrr_m2pam_prs_beta_rest.png', dpi=500)
