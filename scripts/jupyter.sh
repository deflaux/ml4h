...
-v /data/:/data/ \
-p 0.0.0.0:${PORT}:${PORT} \
${DOCKER_IMAGE} /bin/bash -c "apt update ; apt install git -y ; pip install -e /home/${USER}/ml; jupyter notebook --no-browser --ip=0.0.0.0 --port=${PORT} --NotebookApp.token= --allow-root --notebook-dir=/home/${USER}"
