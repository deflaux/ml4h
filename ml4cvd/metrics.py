...
def asymmetric_myocardium(y_true, y_pred):
    ...
    top_over = (K.maximum(myocardium_true, 0) * K.maximum(myocardium_true - myocardium_pred, 0.0)) / 6000.0
    top_under = (K.maximum(myocardium_true, 0) * K.maximum(myocardium_pred - myocardium_true, 0.0)) / 16000.0
    y_pred /= K.sum(y_pred, axis=-1, keepdims=True)
    y_pred = K.clip(y_pred, K.epsilon(), 1 - K.epsilon())
    loss = y_true * K.log(y_pred) * [1.0, 100.0, 100.0]
    loss = -K.sum(loss, -1)
...
def y_true_cubed_times_mse(y_true, y_pred):
    return K.maximum(2*y_true, 1.0)*K.maximum(2*y_true, 1.0)*K.maximum(2*y_true, 1.0)*mean_squared_error(y_true, y_pred)
